...
with st.sidebar:
    selection = st.radio('Select what you want to do', ('Summarize a passage', 'Find sentiment of text', 'Find genre of text'))

if selection == 'Summarize a passage':
    passage = str(st.text_area('Enter the passage you want to summarize'))
    if st.button('Summarize'):
        st.write(summarize(passage))

if selection == 'Find sentiment of text':
    line = str(st.text_area('Enter the statement you want to find the sentiment of'))
    if st.button('Analyze'):
        st.write(sentiment(line))

if selection == 'Find genre of text':
